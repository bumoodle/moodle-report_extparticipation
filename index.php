...
$url = new moodle_url('/report/extparticipation/index.php', array('id'=>$id));
...
require_capability('report/extparticipation:view', $context);

add_to_log($course->id, "course", "report extparticipation", "report/extparticipation/index.php?id=$course->id", $course->id);

$strextparticipation = get_string('extparticipationreport', 'report_extparticipation');
...
$PAGE->set_title($course->shortname .': '. $strextparticipation);
...
echo '<form class="extparticipationselectform" action="index.php" method="get"><div>'."\n".
...
$baseurl =  $CFG->wwwroot.'/report/extparticipation/index.php?id='.$course->id.'&amp;roleid='
...
if (!empty($instanceid) && !empty($roleid)) {
    ...
    $table = new flexible_table('course-extparticipation-'.$course->id.'-'.$cm->id.'-'.$roleid);
    $table->course = $course;
    ...
    echo '<div id="extparticipationreport">' . "\n";
    ...
    foreach ($users as $u) {
        ...
        $table->add_data($data, (!empty($u->count)) ? 'correct' : 'incorrect');
    }
    ...
    $PAGE->requires->js_init_call('M.report_extparticipation.init');
}
